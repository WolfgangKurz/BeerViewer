...
namespace BeerViewer.Models.Quest
{
	...
	internal class TrackerEnemyShip
	{
	...
	internal class BattleTracker
	{
		...
		public void ResetEnemy(int[] api_ship_ke, int[] api_ship_ke2 = null)
		{
			...
			for (var i = 0; i < api_ship_ke.Length; i++)
			{
				...
				enemyShips[i].Id = id;
				enemyShips[i].Type = id == -1 ? 0 : Master.Instance.Ships[id].ShipType.Id;
			}
			...
			if (api_ship_ke2 != null)
			{
				...
				for (var i = 0; i < api_ship_ke2.Length; i++)
				{
					...
					enemyShips2[i].Id = id;
					enemyShips2[i].Type = id == -1 ? 0 : Master.Instance.Ships[id].ShipType.Id;
				}
			}
		}
