...
※ 대피한 함선 제외";
		...
		public override double Calc(Fleet[] fleets)
		{
			...
			var admiralScore = Math.Ceiling(DataStorage.Instance.Homeport.Admiral.Level * 0.4);

			var isCombined = 1 < fleets.Count()
				&& (Settings.IsViewRangeCalcIncludeFirstFleet.Value && Settings.IsViewRangeCalcIncludeSecondFleet.Value);
		...
		private Ship[] GetTargetShips(Fleet[] fleets)
		{
			...
			if (Settings.IsViewRangeCalcIncludeFirstFleet.Value && Settings.IsViewRangeCalcIncludeSecondFleet.Value)
				return fleets.SelectMany(x => x.Ships).ToArray();

			if (Settings.IsViewRangeCalcIncludeFirstFleet.Value)
				return fleets.First().Ships;

			if (Settings.IsViewRangeCalcIncludeSecondFleet.Value)
	...
	{
		public override sealed string Id => "ViewRange.Type1";
	...
	{
		public override sealed string Id => "ViewRange.Type2";
	...
	{
		public override sealed string Id => "ViewRange.Type3";
...
+ (진수부 레벨을 5의 배수로 올림 × -0.61)";
		...
		public override double Calc(Fleet[] fleets)
		{
			...
			var level = (((DataStorage.Instance.Homeport.Admiral.Level + 4) / 5) * 5);
	...
	{
		public override sealed string Id => "ViewRange.Type4";
	...
	{
		public override sealed string Id => "ViewRange.Type5";
	...
	{
		public override sealed string Id => "ViewRange.Type6";
